...
if (!defined("NODE_NAME")) {
    ...
    define("NODE_DEFAULT_STRUCTURES", [
        "Backup" => "Zipped backup states",

        "Config" => [
            "App" => "Application configuration",
            "Cache" => "Cache configuration",
            "Core" => "Core application configuration files",
            "Database" => "Database connections",
            "Env" => "Environment-specific configuration",
            "Mail" => "Email configuration",
            "Module" => "Module or plugin configuration",
            "Queue" => "Queue drivers",
        ],

        "Console" => [
            "Command" => "CLI commands",
            "Kernel" => "CLI bootstrap and dispatcher",
            "Schedule" => "Scheduled tasks",
        ],

        "Database" => [
            "Connection" => "Database configuration and connection pools",
            "Factory" => "Model factories for seeding and testing",
        ...
        "Extension" => [
            ...
            "Plugin" => "Pluggable feature modules",
            "Shortcode" => "Content macros",
            "Theme" => "Presentation overrides",
        ],

        "Git" => [
            "Node" => "Node.php project repository",
            "Project" => "All excluding Node.php",
        ],
        ...
        "Log" => [
            ...
            "Exception" => "Unhandled exception dumps",
        ...
        "Primitive" => [
            ...
            "Function" => [
                ...
                "Hook" => "Extension points for plugins or modules",
            ...
            "Interface" => [
                "Behavioral" => [
                    ...
                    "Listener" => "Event listener contract",
                    "Observer" => "Observer pattern contract",
            ...
            "Trait" => [
                ...
                "SoftDeletes" => "Soft deletion behavior",
                "Timestampable" => "Automatic timestamp management",
            ],
            "Class" => [
                "Abstract" => [
                    "Base" => [
                ...
                "Final" => [
                    ...
                    "Coordination" => [
                        "Event" => "Application event object",
                        "EventDispatcher" => "Concrete event publisher",
                        "Job" => "Queueable job implementation",
                        ...
                        "Routes" => [
                            ...
                            "Console" => "Console route definitions",
                    ...
                    "Presentation" => [
                        "Controller" => [
                            ...
                            "Api" => "API-specific controller",
                            "Page" => "Web page controller",
                            "Resource" => "RESTful resource controller",
                        ],
                        "Endpoint" => "Concrete public API endpoint",
                        "Filter" => "Request filtering implementation",
                        "Http" => [
                            "Request" => "Inbound HTTP request objects",
        ...
        "Template" => [
            "Blade" => "Blade-style templates (optional compatibility)",
...
if (!function_exists("f")) {
    ...
    function _node_generate_boilerplate(string $call, string $name, string $LOCAL_PATH): array
    {
        ...
        # Check if we're in Primitive directory structure
        $isPrimitive = ($parts[0] ?? "") === "Primitive";

        # Determine the actual type (Class, Interface, Trait, Enum, Function)
        if ($isPrimitive && isset($parts[1])) {
            $type = $parts[1];
            $subtype = $parts[2] ?? "";
            $category = $parts[3] ?? "";
        } else {
        ...
        # Build namespace (functions never have namespace)
        $namespace = "";
        if ($type !== "Function" && !empty($parts)) {
            # For Abstract/Final classes in Primitive/Class directory
            if (in_array("Abstract", $parts, true)) {
                # Keep Abstract in namespace but remove from class keyword
                $namespace = "namespace " . implode("\\", $parts) . ";\n\n";
            } elseif (in_array("Final", $parts, true)) {
                # Keep Final in namespace but remove from class keyword
                $namespace = "namespace " . implode("\\", $parts) . ";\n\n";
            } else {
                $namespaceParts = array_map("ucfirst", $parts);
                $namespace = "namespace " . implode("\\", $namespaceParts) . ";\n\n";
            }
        }

        # Determine keyword and class name
        $keyword = match ($type) {
            "Interface" => "interface",
            "Trait" => "trait",
            "Function" => "function",
            "Enum" => "enum",
            "Class" => match (true) {
                in_array("Abstract", $parts, true) => "abstract class",
                in_array("Final", $parts, true) => "final class",
                default => "class",
            },
            default => "class",
        };

        # Generate appropriate class/interface name
        $className = match (true) {
            $type === "Enum" => $name,
            $type === "Function" => "{$name}()",
            $keyword === "interface" && $leaf !== "Interface" => "{$name}{$leaf}",
            $keyword === "trait" && $leaf !== "Trait" => "{$name}{$leaf}",
            $leaf === $type || $leaf === $subtype || $leaf === $category => $name,
            default => "{$name}{$leaf}",
        };

        # Add suffix for certain types
        if (
            $type === "Class" &&
            in_array($subtype, [
                "Controller",
            ...
            if (!str_ends_with($className, $subtype)) {
                $className .= $subtype;
            }
        }

        # Generate appropriate body based on structure
        $body = match (true) {
            # Interfaces
        ...
        $useStatements = "";
        if ($type === "Event") {
            $useStatements = "use DateTimeInterface;\n\n";
        }

        if (str_contains($body, "DateTimeInterface") && !str_contains($namespace, "use DateTimeInterface")) {
            $useStatements = "use DateTimeInterface;\n\n";
        }

        # Insert use statements after namespace
        if ($useStatements && $namespace) {
            $namespace = str_replace("\n\n", "\n\n" . $useStatements, $namespace);
        }

        # Special handling for functions (remove namespace)
        if ($type === "Function") {
            $namespace = "";
            # Add function signature for functions
            $returnType = match ($leaf) {
            ...
            # Update the function signature in the body
            if (str_starts_with($body, "():")) {
                $body = str_replace("():", $returnType . "\n{", $body);
            }
        }
